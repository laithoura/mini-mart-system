    ...
    public partial class USold : UserControl, IMessageType
    {
        ...
        private void DoClick(object sender, EventArgs e)
        {
            if (sender == btnAdd)
            {
                ...
                double amount = 0;
                int currentNumber = dgvSale.Rows.Count + 1;
                string productName = cboProductName.Text.Trim();

                int qty = 0, importID = 0;
                int.TryParse(cboImportID.SelectedValue.ToString(),out importID);
                int.TryParse(txtQty.Text.Trim(),out qty);
                qty = (qty == 0) ? 1 : qty;

                float unitPrice = 0f, discount = 0f, tax = 0f;
                float.TryParse(txtPrice.Text.Trim(), out unitPrice);                              
                float.TryParse(txtDiscount.Text.ToString(), out discount);
                float.TryParse(txtTax.Text.ToString(), out tax);               
                
                float subTotal = qty * unitPrice;
                float totalDiscount = subTotal * discount / 100;
                float totalTax = subTotal * tax / 100;

                amount =  subTotal - totalDiscount + totalTax;

                dgvSale.Rows.Add(currentNumber, importID, proID, productName, qty, unitPrice, discount, tax, amount);
            ...
            else if (sender == btnUpdate)
            {
                ...
                double amount = 0;
                int currentNumber = int.Parse(dgvSale.Rows[index].Cells[0].Value.ToString());
                string productName = cboProductName.Text.Trim();
                ...
                amount = subTotal - totalDiscount + totalTax;                
                dgvSale.Rows[index].SetValues(currentNumber, importID, proID, productName, qty, unitPrice, discount, tax, amount);
            ...
            {                
                 ...
                 if (verifyResult == DialogResult.Yes)
                 {
                     ...
                     if (!insertSold)
                     {
                         ...
                         MessageSuccess("Sale was saved unsuccessfully", "Sell Product");
                         return;
                     }
                     ...
                     if (insertSold && insertDetails && CutStockOfSaleProduct())
                     {
                         MessageSuccess("Sale was saved successful", "Sale Product");
                     ...
                     else if (!insertDetails) /*Insert into Sold Detail is error.*/
                     {
                         ...
                         {
                             MessageError(ex.Message, "Sale Product");
                         }
                         finally
                         {
                             try
        ...
        private bool CutStockOfSaleProduct()
        {
            bool success = false;
            int updateNumber = 0;
            con.Open();
            cmd = new SqlCommand("UPDATE ImportDetail SET soldQty = (soldQty + @qty) WHERE impID = @impID AND proID = @proID;", con);
            cmd.Parameters.Add("@qty", SqlDbType.Int);
            cmd.Parameters.Add("@impID", SqlDbType.Int);
            cmd.Parameters.Add("@proID", SqlDbType.Int);            
            
                try
                {
                    foreach (DataRow row in dataTableSoldDetail.Rows)
                    {
                        cmd.Parameters["@qty"].Value = (int)row["qty"];
                        cmd.Parameters["@impID"].Value = (int)row["impID"];
                        cmd.Parameters["@proID"].Value = (int)row["proID"];
                ...
                {
                    MessageError(ex.Message, "Cut Stock");
                }
                finally
                {
                    try
                ...
                /*End of Cutting Stock*/
                
                if (updateNumber == dataTableSoldDetail.Rows.Count)
                {
                    success = true;
                    if(dataTableSoldDetail != null) dataTableSoldDetail.Dispose();
                }
            return success;
        }
